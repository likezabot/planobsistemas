-- =====================================================================
-- Restaurar create_public_order completa + rate-limit global no topo
-- =====================================================================
...
DECLARE
    ...
    c_global_window CONSTANT INTEGER := 1;
...
-- =====================================================================
-- Restaurar overloads internas (uuid, text) de print_jobs
-- (já estavam revogadas para anon/PUBLIC; mantêm acesso só authenticated/service_role)
-- =====================================================================
...
BEGIN
  UPDATE public.print_jobs SET status='printing', agent_id=p_agent_id, claimed_at=now(), attempts=attempts+1
...
DECLARE v_updated integer;
BEGIN
  UPDATE public.print_jobs SET status='printed', printed_at=now()
   WHERE id=p_job_id AND status='printing' AND agent_id=p_agent_id;
  GET DIAGNOSTICS v_updated = ROW_COUNT;
  RETURN v_updated > 0;
END; $$;
...
DECLARE v_updated integer;
BEGIN
  UPDATE public.print_jobs SET status='failed', last_error=p_error
   WHERE id=p_job_id AND status IN ('printing','pending') AND (agent_id=p_agent_id OR agent_id IS NULL);
  GET DIAGNOSTICS v_updated = ROW_COUNT;
  RETURN v_updated > 0;
END; $$;
