-- Update open_table_order to set customer_name
...
BEGIN
    ...
    INSERT INTO public.orders (
        restaurant_id, tenant_id, service_mode, table_id, opened_by, status, payment_status, subtotal_cents, total_cents, customer_name
    ) VALUES (
        _restaurant_id, v_tenant_id, 'table', _table_id, auth.uid(), 'new', 'open', 0, 0, 'Mesa ' || v_table_name
...
-- Update create_counter_order to set customer_name
...
BEGIN
    ...
    INSERT INTO public.orders (
        restaurant_id, tenant_id, service_mode, opened_by, status, payment_status, subtotal_cents, total_cents, customer_name
    ) VALUES (
        _restaurant_id, v_tenant_id, 'counter', auth.uid(), 'new', 'open', 0, 0, 'Balcão'
    ) RETURNING id INTO v_order_id;
