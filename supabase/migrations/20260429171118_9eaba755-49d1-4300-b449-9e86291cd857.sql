-- =========================================================
-- 1) Rate limit table for public checkout attempts
-- =========================================================
CREATE TABLE IF NOT EXISTS public.public_order_attempts (
...
CREATE INDEX IF NOT EXISTS idx_public_order_attempts_lookup
  ON public.public_order_attempts (restaurant_id, normalized_phone, created_at DESC);

ALTER TABLE public.public_order_attempts ENABLE ROW LEVEL SECURITY;

-- Only owner/manager/support can read attempts of their own restaurant
CREATE POLICY "public_order_attempts_select_admins"
ON public.public_order_attempts
FOR SELECT
TO authenticated
USING (public.has_any_role_in_restaurant(auth.uid(), restaurant_id, ARRAY['owner','manager','support']::app_role[]));

-- Nobody (anon/auth) can insert/update/delete via REST.
-- Inserts happen only inside the SECURITY DEFINER RPC.

-- =========================================================
-- 2) create_public_order with rate limit
-- =========================================================
...
BEGIN
    SELECT id, tenant_id, inventory_enabled, inventory_mode 
    INTO v_restaurant_id, v_tenant_id, v_inventory_enabled, v_inventory_mode
    ...
    -- Idempotency check FIRST: same key returns same order without counting as a new attempt
    ...
    -- Rate limit: count distinct idempotency_keys (= real attempts) by this phone in this restaurant
    -- in the last window. Same idempotency_key never reaches here (caught above).
    ...
    IF v_recent_attempts >= c_max_attempts THEN
        ...
        INSERT INTO public.audit_log (
            tenant_id, restaurant_id, action, entity, payload
        ) VALUES (
            v_tenant_id, v_restaurant_id, 'public_order.rate_limited', 'public_order_attempts',
            jsonb_build_object(
                'phone_tail', right(v_normalized_phone, 4),
                'attempts_in_window', v_recent_attempts,
                'window_minutes', c_window_minutes,
    ...
    -- Record attempt (allowed). Done before order creation so it counts even if downstream fails.
    INSERT INTO public.public_order_attempts (
        restaurant_id, normalized_phone, idempotency_key, blocked
    ) VALUES (
...
-- =========================================================
-- 3) Recovery: reset stuck print jobs
-- =========================================================
CREATE OR REPLACE FUNCTION public.reset_stuck_print_jobs(
...
DECLARE
  v_count INTEGER;
BEGIN
  IF NOT public.has_any_role_in_restaurant(auth.uid(), p_restaurant_id, ARRAY['owner','manager']::app_role[]) THEN
    RAISE EXCEPTION 'Acesso negado.';
  END IF;

  IF p_stuck_minutes < 1 THEN
    p_stuck_minutes := 5;
  END IF;
  ...
  WITH updated AS (
    ...
    SET status = 'pending', agent_id = NULL, claimed_at = NULL,
        last_error = COALESCE(last_error, '') || ' | reset by reset_stuck_print_jobs at ' || now()::text
  ...
  IF v_count > 0 THEN
    INSERT INTO public.audit_log (restaurant_id, user_id, action, entity, payload)
    VALUES (
      p_restaurant_id, auth.uid(), 'print_jobs.reset_stuck', 'print_jobs',
      jsonb_build_object('count', v_count, 'stuck_minutes', p_stuck_minutes)
    );
  END IF;
...
REVOKE ALL ON FUNCTION public.reset_stuck_print_jobs(UUID, INTEGER) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.reset_stuck_print_jobs(UUID, INTEGER) TO authenticated;
