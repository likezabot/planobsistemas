...
BEGIN
    ...
    LOOP
        ...
        -- Fetch product data including stock fields
        ...
        IF v_product_type IN ('variable','pizza') AND v_variation_id IS NOT NULL THEN
            ...
            -- Inventory check: Variation level (Advanced mode only)
        ...
        IF v_product_type = 'pizza' THEN
            ...
            FOREACH v_flavor_id IN ARRAY v_pizza_flavor_ids LOOP
                ...
                -- Inventory check: Flavor level (Advanced mode only)
                IF v_inventory_enabled AND v_inventory_mode = 'advanced' AND v_track_stock THEN
                    IF v_stock_quantity < v_quantity AND NOT v_allow_out_of_stock_sale THEN
        ...
        -- Options items validation and inventory
        IF array_length(v_selected_options, 1) > 0 THEN
            FOREACH v_option_id IN ARRAY v_selected_options LOOP
                SELECT oi.name, oi.price_cents, oi.active, oi.track_stock, oi.stock_quantity, oi.allow_out_of_stock_sale 
                INTO v_option_name, v_option_price, v_product_active, v_track_stock, v_stock_quantity, v_allow_out_of_stock_sale
                FROM public.option_items oi
                ...
                -- Inventory check: Option level (Advanced mode only)
                IF v_inventory_enabled AND v_inventory_mode = 'advanced' AND v_track_stock THEN
                    IF v_stock_quantity < v_quantity AND NOT v_allow_out_of_stock_sale THEN
                        RAISE EXCEPTION 'Estoque insuficiente para o opcional: %', v_option_name;
                    END IF;
                    PERFORM public.update_stock(v_restaurant_id, 'option_item', v_option_id, v_stock_quantity - v_quantity, 'sale', v_order_id, NULL);
                END IF;
