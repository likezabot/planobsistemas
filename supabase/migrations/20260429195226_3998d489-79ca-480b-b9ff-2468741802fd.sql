...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE restaurant_id = _restaurant_id AND user_id = v_user_id
    ) THEN
        RAISE EXCEPTION 'Acesso negado.';
    END IF;

    SELECT tenant_id, inventory_enabled, inventory_mode, pizza_module_enabled
    INTO v_tenant_id, v_inventory_enabled, v_inventory_mode, v_pizza_enabled
    ...
    WHERE restaurant_id = _restaurant_id AND idempotency_key = _idempotency_key;
    ...
    INSERT INTO public.orders (
        ...
        v_tenant_id, _restaurant_id, trim(_customer_name), v_normalized_phone, _order_type,
    ...
    LOOP
        ...
        IF v_inventory_enabled AND v_track_stock THEN
            ...
            END IF;
            PERFORM public.update_stock(_restaurant_id, 'product', v_product_id, v_stock_quantity - v_quantity, 'sale', v_order_id, NULL);
        END IF;
        ...
        IF v_product_type IN ('variable','pizza') AND v_variation_id IS NOT NULL THEN
            ...
            IF v_inventory_enabled AND v_inventory_mode = 'advanced' AND v_track_stock THEN
                ...
                END IF;
                PERFORM public.update_stock(_restaurant_id, 'variant', v_variation_id, v_stock_quantity - v_quantity, 'sale', v_order_id, NULL);
            END IF;
        ...
        IF v_product_type = 'pizza' THEN
            ...
            FOREACH v_flavor_id IN ARRAY v_pizza_flavor_ids LOOP
                ...
                WHERE pf.id = v_flavor_id
                  ...
                  AND pf.restaurant_id = _restaurant_id;
                ...
                IF v_inventory_enabled AND v_inventory_mode = 'advanced' AND v_track_stock THEN
                    ...
                    END IF;
                    PERFORM public.update_stock(_restaurant_id, 'pizza_flavor', v_flavor_id, v_stock_quantity - v_quantity, 'sale', v_order_id, NULL);
                END IF;
        ...
        IF array_length(v_selected_options, 1) > 0 THEN
            FOREACH v_option_id IN ARRAY v_selected_options LOOP
                ...
                WHERE oi.id = v_option_id AND og.restaurant_id = _restaurant_id;
